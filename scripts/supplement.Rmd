...
ggplot2::ggsave(here::here("figures", "S1Fig.png"), plot = figS1, device = "png", width = 5.2, height = 5.5, units = "in", dpi = 300)
```
...
redvTOF <- pruneddata %>%
  ...
  scale_x_continuous(breaks = seq(0, 70, 10)) +
  theme_cowplot(font_size = 10, rel_small = 8/10) +
  labs(x="Time (hours)", y = "Median red/TOF")
...
redvEXT <- pruneddata %>%
  ...
  scale_x_continuous(breaks = seq(0, 70, 10)) +
  theme_cowplot(font_size = 10, rel_small = 8/10) +
  labs(x="Time (hours)", y = "Median red/EXT")
...
qvolume <- pruneddata %>%
  ...
  scale_point_color_gradient(name = "% Quiescent", low = "black", high = "red",
                             ...
                             labels = c("10","20","40","60","80")) +
  theme_cowplot(font_size = 10, rel_small = 8/10) + guides(point_color = F) +
  labs(y ="Time (hours)", x = expression(paste("log Volume (", mu,m^3,")"))) 
