...
qlength <- pruneddata %>%
  ...
  dplyr::ungroup() %>% 
  ggplot() +
  aes(x = rev(TOF), y = factor(rev(hour), levels = c(40,39,38,37,36,35,34,33,32)), 
      group = rev(timepoint), point_color = `% Quiescent`) +
...
qwidth <- pruneddata %>%
  ...
  dplyr::ungroup() %>% 
  ggplot() +
  aes(x = rev(norm.EXT), y = factor(rev(hour), levels = c(40,39,38,37,36,35,34,33,32)), 
      group = rev(timepoint), point_color = `% Quiescent`) +
  geom_density_ridges(alpha = 0.2, quantile_lines = T, quantiles = 5, jittered_points = T, 
                      position = position_points_sina(seed = 2),
                      point_alpha = 0.4, point_size = 0.4, scale = 0.99) +
...
qvolume <- pruneddata %>%
  ...
  dplyr::ungroup() %>% 
  ggplot() +
  aes(x = rev(volume), y = factor(rev(hour), levels = c(40,39,38,37,36,35,34,33,32)), 
      group = rev(timepoint), point_color = `% Quiescent`) +
  geom_density_ridges(alpha = 0.2, quantile_lines = T, quantiles = 5, jittered_points = T, 
                      position = position_points_sina(seed = 2),
                      point_alpha = 0.4, point_size = 0.4, scale = 0.99) +
